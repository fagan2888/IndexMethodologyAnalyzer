...
esgIndexTotal <- c(100,100,100,100)
...
```{r warning=F, echo=F}
universesus <- universe %>% filter(grepl(esgKeywords,Primary.Prospectus.Benchmark))
MSCIsus <- MSCI %>% filter(grepl(esgKeywords,Primary.Prospectus.Benchmark))
FTSEsus <- FTSE %>% filter(grepl(esgKeywords,Primary.Prospectus.Benchmark))
STOXXsus <- STOXX %>% filter(grepl(esgKeywords,Primary.Prospectus.Benchmark))
SPDJsus <- SPDJ %>% filter(grepl(esgKeywords,Primary.Prospectus.Benchmark))

esgCount <- c(length(MSCIsus$Name),length(FTSEsus$Name),length(STOXXsus$Name),length(SPDJsus$Name))
esgPercent <- esgCount/sum(esgCount)
esgAUM <- c(sumF(MSCIsus$Fund.Size.USD),sumF(FTSEsus$Fund.Size.USD),sumF(STOXXsus$Fund.Size.USD),sumF(SPDJsus$Fund.Size.USD))
...
plotTimeSeriesForAllScope <- function(universe, esgU, envU, keyword="",  yearLimit=1930) {
  ...
  colnames(esgDF) <- c('year', 'esgCount')
  colnames(envDF) <- c('year', 'envCount')
  yearDF %>%
    full_join(esgDF) %>%
    full_join(envDF) %>%
    ggplot() + 
    geom_line(aes(x=year, y=count), color="green") + 
    geom_line(aes(x=year, y=esgCount), color='red') + 
    geom_line(aes(x=year, y=envCount), color='blue') + 
    scale_x_continuous(breaks=seq(yearLimit, 2016, 5))
}
...
```{r warning=F, echo=F}
plotTimeSeriesForUniverse(universe)
```

## example: ESG time series
```{r warning=F, echo=F}
plotTimeSeriesForUniverse(universesus)
```

## example: MSCI ESG time series
```{r warning=F, echo=F}
plotTimeSeriesForUniverse(universesus, msciKeyword)
```

## example: SPDJ time series
```{r warning=F, echo=F}
plotTimeSeriesForUniverse(universe, spdjKeyword)
```
\newpage
...
```{r warning=F, echo=F}
institutionChoice(blackrockKeyword, universesus)
```

```{r warning=F, echo=F}
plotInstitutionChoice(blackrockKeyword, universesus)
```
...
## exsample: HSBC
```{r warning=F, echo=F}
institutionChoice('HSBC', universe)
```

```{r warning=F, echo=F}
plotInstitutionChoice('HSBC', universe)
```
\newpage
...
```{r warning=F, echo=F}
plotUniverseByBrandName(universesus, floor=2)
```
...
```{r warning=F, echo=F}
plotUniverseByBrandName(MSCIsus, floor=0)
```
