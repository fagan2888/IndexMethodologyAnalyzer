...
countTable %>%
  ...
  geom_bar(stat="identity") +
  geom_label(fill='white',aes(label=paste(as.character(round(AUM/1000000000, digits=2)),"B"))) +
  fillTheme
```
...
esgTable %>%
  ...
  geom_bar(stat="identity") +
  geom_label(fill='white',aes(label=paste(as.character(round(esgAUM/1000000000, digits=2)),"B"))) +
  fillTheme
```
...
envTable %>%
  ...
  geom_bar(stat="identity") +
  geom_label(fill='white',aes(label=paste(as.character(round(envAUM/1000000000, digits=2)),"B"))) +
  fillTheme
```
...
data.frame(esg=esgIndexTotal, env=envIndexTotal, ratio=esgIndexTotal/envIndexTotal) %>%
  ...
  geom_bar(stat="identity") +
  geom_label(fill='white',aes(label=round(ratio, digits=4))) +
  fillTheme
```

## Count of ESG index in use VS Count of Environment index in use
```{r warning=F, echo=F}
esgTable %>%
